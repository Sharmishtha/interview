...
| ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Cannot find module '@anthropic-ai/sdk'` on `npm run build` | `node_modules` predates your last `git pull`. Run `npm install`. The follow-on errors in `llm-evaluator.ts` (`implicitly has an 'any' type`, `Property 'quotes' does not exist on type '{}'`) are the same cause: with `zod` unresolvable, the inferred schema type collapses to `{}`. |
